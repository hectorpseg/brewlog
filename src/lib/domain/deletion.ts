...
export function describeDeletion(
  kind: DeletionKind,
  counts: { brews?: number; observations?: number; experiments?: number },
  ...
  switch (kind) {
    ...
    case "brew":
      return {
        ...
        body: `${n(counts.observations, "tasting note set", "tasting note sets")} will be permanently deleted. Linked experiments are kept but unlinked. The dose is handed back to the coffee's remaining weight. This cannot be undone.`,
        confirm: "Delete brew",
      };
    case "session":
      return {
